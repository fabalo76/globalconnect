...
import com.uic.uicpaymentapp.cardreader.nexgo.EmvConfigBuilder
import com.uic.uicpaymentapp.cardreader.nexgo.CapkEnvironment
import com.uic.uicpaymentapp.cardreader.nexgo.EmvUtils
import com.uic.uicpaymentapp.cardreader.nexgo.NexgoApi
import com.uic.uicpaymentapp.dao.AppContainer
import com.uic.uicpaymentapp.dao.AppDataContainer
import com.uic.uicpaymentapp.uicpos.pos.host.AcquirerSslCache
import com.uic.uicpaymentapp.uicpos.pos.host.BundledCertificates
import com.uic.uicpaymentapp.uicpos.pos.host.loadCertificatesFromStream
import com.uic.uicpaymentapp.transaction.BrandingAnimationCache
...
class UICApplication : Application() {
    ...
    override fun onCreate() {
        ...
        applicationScope.launch {
            ...
            PendingUpdateManager.restoreFromPrefs(this@UICApplication)
            loadBundledCaCertificates()
            AcquirerSslCache.build(tmsDatabase.IPTab, BundledCertificates.get())
            container = AppDataContainer(this@UICApplication, tmsDatabase, appVersion)
            ...
            EmvUtils.getCapkList(this@UICApplication, capkEnv)
                ?.let { NexgoApi.applyCapkList(it) }
                ?: Log.w("UICApplication", "No CAPK keys loaded for env=$capkEnv")
            ...
            if (tmsDatabase.Terminal.isNotEmpty()) {
                ...
                launch(Dispatchers.Main) {
                    BrandingAnimationCache.prewarm(this@UICApplication, tmsDatabase)
                }
            }
            _appInitialized.value = true

            // On first install (no cached params) ask UIC Home to download them.
            // TmsParamsReceiver handles the reply and sets paramsReadyFlow = true.
            if (tmsDatabase.Terminal.isEmpty()) {
                requestParamsFromUicHome()
            }
        }
    }

    /**
     * Broadcasts a parameter request to the UIC Home application.
     *
     * UIC Home discovers this app via the PAY_APP intent filter, downloads the
     * parameter JSON from the TMS server over MQTT + HTTP, and replies with
     * ACTION_PARAMS_READY (received by [TmsParamsReceiver]).
     *
     * Safe to call at any time — UIC Home guards against concurrent downloads.
     */
    fun requestParamsFromUicHome() {
        // Discover whichever flavor of the home app is installed rather than hardcoding.
        ...
        val probe     = Intent("com.uic.home.ACTION_REQUEST_PARAMS")
        val receivers = packageManager.queryBroadcastReceivers(probe, 0)
        if (receivers.isEmpty()) {
            Log.w("UICApplication", "ACTION_REQUEST_PARAMS: no UIC Home app installed — skipping")
            return
        }
        for (ri in receivers) {
            val homePackage = ri.activityInfo.packageName
            Log.i("UICApplication", "Broadcasting ACTION_REQUEST_PARAMS → $homePackage")
            sendBroadcast(Intent("com.uic.home.ACTION_REQUEST_PARAMS").apply {
                `package` = homePackage
    ...
    private fun loadBundledCaCertificates() {
        try {
            ...
            if (certs.isEmpty()) {
                Log.i("UICApplication", "trusted_cas.crt is empty — no bundled CA certificates loaded")
            } else {
                BundledCertificates.set(certs)
                Log.i("UICApplication", "Loaded ${certs.size} bundled CA certificate(s) from trusted_cas.crt:")
                certs.forEachIndexed { i, cert ->
                    Log.i("UICApplication", "  [$i] subject=${cert.subjectX500Principal.name}")
                    Log.i("UICApplication", "  [$i] issuer=${cert.issuerX500Principal.name}")
                    Log.i("UICApplication", "  [$i] validity=${cert.notBefore} → ${cert.notAfter}")
                }
            }
        } catch (e: Exception) {
            Log.w("UICApplication", "Could not load bundled CA certificates", e)
        }
    }
     ...
     *
     * Parameters are downloaded from the TMS server by the UIC Home application and
    ...
    private fun loadTMSDatabase(): TMSDATA {
        ...
        if (!storedFile.exists()) {
            Log.i("TMSDatabase", "No stored TMS params found — waiting for download from UIC Home")
            return TMSDATA()
        }
    ...
    fun applyTmsUpdate(newDatabase: TMSDATA, persistToDisk: Boolean = true): Boolean {
        ...
        applicationScope.launch(Dispatchers.Main) {
            BrandingAnimationCache.prewarm(this@UICApplication, newDatabase)
        }
    ...
    companion object {
        @Volatile
        private var _instance: UICApplication? = null
        ...
        val instanceOrNull: UICApplication?
            get() = _instance

        val instance: UICApplication
            get() = _instance ?: error("UICApplication not created yet")
