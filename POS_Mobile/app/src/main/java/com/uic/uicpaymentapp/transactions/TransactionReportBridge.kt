package com.uic.uicpaymentapp.transactions
...
import com.uic.uicpaymentapp.settlement.SettlementTarget
...
object TransactionReportBridge {
    ...
    private fun reportTransactionInternal(
        ...
        val payload = transaction.toReportJson(event)
    ...
    private fun sendPayload(context: Context, payload: JSONObject, transactionId: String?, batchId: String?) {
        ...
        if (receivers.isEmpty()) {
            Log.w(TAG, "ACTION_REPORT_TRANSACTION: no UIC Home app installed")
            return
        }
...
private fun Transaction.toReportJson(event: TransactionReportEvent): JSONObject {
    ...
    return JSONObject()
        ...
        .put("currencyCode", SysParam.getInstance().CurrCode.ifBlank { "USD" })
