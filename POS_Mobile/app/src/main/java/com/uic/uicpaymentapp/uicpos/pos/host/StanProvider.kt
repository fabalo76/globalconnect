...
object StanProvider {
    ...
    fun setStanValue(value: Int) {
        ...
        UICApplication.instance.getSharedPreferences(PREFERENCES, Context.MODE_PRIVATE)
            .edit().putInt(KEY_STAN, clamped).apply()
        Log.d(TAG, "STAN manually set to ${String.format("%06d", clamped)}")
    }

    fun currentStan(): String {
        val preferences = UICApplication.instance.getSharedPreferences(PREFERENCES, Context.MODE_PRIVATE)
    ...
    private fun incrementAndPersist(): Int {
        val preferences = UICApplication.instance.getSharedPreferences(
            PREFERENCES,
