...
/**
 * Receives [ACTION_PRE_INSTALL_CHECK] from xTMSAgent before it installs an APK update for this app.
 *
 * Responds with [ACTION_PRE_INSTALL_RESPONSE]:
 *  - `proceed = true`  → terminal is idle; xTMSAgent may install immediately.
 *  - `proceed = false` → terminal is busy; xTMSAgent must defer the install.
