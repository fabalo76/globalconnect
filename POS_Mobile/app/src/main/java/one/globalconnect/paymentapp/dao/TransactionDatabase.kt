...
import one.globalconnect.paymentapp.settlement.storage.SettlementState
import one.globalconnect.paymentapp.settlement.storage.SettlementStateDao
import one.globalconnect.paymentapp.transaction.PendingReversal
import one.globalconnect.paymentapp.transaction.ResetState
...
abstract class TransactionDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        internal val MIGRATION_11_12 = object : Migration(11, 12) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE pending_reversals ADD COLUMN paymentPlanQueryResponse TEXT NOT NULL DEFAULT ''")
            }
        }

        internal val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `transaction` ADD COLUMN posTransactionId TEXT NOT NULL DEFAULT ''")
            }
        }
