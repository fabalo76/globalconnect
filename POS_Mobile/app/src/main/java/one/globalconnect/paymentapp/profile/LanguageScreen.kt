...
private fun getLanguage(savedLanguage: String, items: List<String>): String {
    val language = when (savedLanguage) {
        "en" -> items[1]
        "es" -> items[2]
        else -> GlobalConnectPaymentApplication.instance.resources.configuration.locales[0]
    }
...
private fun getLocate(language: String, items: List<String>): String {
    return when (language) {
        items[1] -> "en"
        items[2] -> "es"
        else -> GlobalConnectPaymentApplication.instance.resources.configuration.locales[0].language; // Use correct locale from resources
