# xTMSAgent ↔ LATAM Payment Application Interaction

This document describes the inter-process communication (IPC) protocol between the **xTMSAgent** launcher application and the **LATAM Payment Application**. All communication uses Android Broadcast Intents.

---

## Overview

xTMSAgent is the central launcher and device management agent on Nexgo POS terminals. It:
...
The payment app is a passive consumer: it requests parameters at startup if none are cached, responds to parameter pushes, and negotiates install windows with xTMSAgent before allowing an APK update to proceed.
...
|--------|-----------|---------|
| `one.globalconnect.xtmsagent.ACTION_REQUEST_PARAMS` | App → Home | Request current TMS parameters |
| `one.globalconnect.xtmsagent.ACTION_PARAMS_READY` | Home → App | Deliver TMS parameters via URI |
| `one.globalconnect.xtmsagent.ACTION_PARAMS_FAILED` | Home → App | Signal parameter download failure |
| `one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_CHECK` | Home → App | Ask if app is idle and ready for update |
| `one.globalconnect.paymentapp.ACTION_PRE_INSTALL_RESPONSE` | App → Home | Consent or defer update |
| `one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_DISMISS` | Home → App | Cancel a pending install task |
...
On startup, `GlobalConnectPaymentApplication.onCreate()` checks whether a valid `TMS_Database` is already loaded. If `tmsDatabase.Terminal` is empty (first run or cache cleared), the app initiates a parameter request.

### Step 1 — App requests parameters

**Sender:** `GlobalConnectPaymentApplication.requestParamsFromXtmsAgent()`  
**Action:** `one.globalconnect.xtmsagent.ACTION_REQUEST_PARAMS`  
**Type:** Explicit broadcast — sent individually to each discovered xTMSAgent package
...
val receivers = packageManager.queryBroadcastReceivers(
    Intent("one.globalconnect.xtmsagent.ACTION_REQUEST_PARAMS"), 0
)
...
### Step 2 — xTMSAgent delivers parameters

**Sender:** xTMSAgent  
**Receiver:** `TmsParamsReceiver` (registered in `AndroidManifest.xml`)  
**Action:** `one.globalconnect.xtmsagent.ACTION_PARAMS_READY`

| Extra | Type | Description |
|-------|------|-------------|
| `params_uri` | String | `content://` FileProvider URI pointing to decompressed TMS JSON |

xTMSAgent downloads the current TMS_Database JSON from the server, writes it to a shared FileProvider path, and broadcasts the URI.
...
5. **If terminal is idle:** calls `GlobalConnectPaymentApplication.applyTmsUpdate(tmsDatabase)` immediately
6. Sets `paramsReadyFlow = true` → triggers UI transition in `MainActivity`

### Error path

**Action:** `one.globalconnect.xtmsagent.ACTION_PARAMS_FAILED`
...
When TMS configuration changes on the server, xTMSAgent receives an MQTT notification and proactively downloads the updated `TMS_Database` without waiting for a request from the payment app. It then delivers the new parameters using the same `ACTION_PARAMS_READY` broadcast described in Step 2 above.
...
```
TMS Server ──MQTT──► xTMSAgent ──ACTION_PARAMS_READY──► Payment App
```
...
When the TMS server publishes a new APK for the payment app, xTMSAgent downloads it and initiates a coordinated install. Because installing an APK terminates the running process, the payment app must confirm it has no open transactions or active operation before xTMSAgent proceeds.

### Step 1 — xTMSAgent requests consent

**Sender:** xTMSAgent  
**Receiver:** `TmsAppUpdateReceiver` (registered in `AndroidManifest.xml`)  
**Action:** `one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_CHECK`

| Extra | Type | Description |
|-------|------|-------------|
| `pkg` | String | Package name of the APK to install (e.g., `one.globalconnect.paymentapp`) |
| `ver` | String | Human-readable version string (e.g., `"2.03"`) |
| `verCode` | Int | Version code (integer build number) |
| `senderPkg` | String | xTMSAgent package name — used to route the response |
...
**Receiver:** xTMSAgent (targeted via `senderPkg`)  
**Action:** `one.globalconnect.paymentapp.ACTION_PRE_INSTALL_RESPONSE`
...
```kotlin
// Pseudo-code
val response = Intent("one.globalconnect.paymentapp.ACTION_PRE_INSTALL_RESPONSE").apply {
...
2. If true: calls `PendingUpdateManager.notifyXtmsAgentCanProceed(context)`
   - Discovers all xTMSAgent instances via `queryBroadcastReceivers(ACTION_PRE_INSTALL_CHECK)`
   - Sends `ACTION_PRE_INSTALL_RESPONSE` with `proceed = true` to each
3. Clears the pending flag

### Step 5 — Install task cancellation

If xTMSAgent cancels the pending install (e.g., update superseded or user cancelled from Home UI):

**Action:** `one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_DISMISS`
...
```
Payment App                     xTMSAgent                    TMS Server
    │                               │                             │
...
Payment App                     xTMSAgent
    ...
    │                               │
    │── ACTION_PRE_INSTALL_RESPONSE►│  (discover all xTMSAgent instances)
    │    (proceed=true)             │
...
<receiver android:name=".tms.TmsParamsReceiver" android:exported="true">
    <intent-filter>
        <action android:name="one.globalconnect.xtmsagent.ACTION_PARAMS_READY" />
        <action android:name="one.globalconnect.xtmsagent.ACTION_PARAMS_FAILED" />
    </intent-filter>
</receiver>

<receiver android:name=".tms.TmsAppUpdateReceiver" android:exported="true">
    <intent-filter>
        <action android:name="one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_CHECK" />
        <action android:name="one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_DISMISS" />
    </intent-filter>
</receiver>
...
<queries>
    <intent>
        <action android:name="one.globalconnect.xtmsagent.ACTION_REQUEST_PARAMS" />
    </intent>
    <intent>
        <action android:name="one.globalconnect.xtmsagent.ACTION_PRE_INSTALL_CHECK" />
    </intent>
...
| File | Role |
|------|------|
| `GlobalConnectPaymentApplication.kt` | Startup: loads cached params, triggers `requestParamsFromXtmsAgent()` |
