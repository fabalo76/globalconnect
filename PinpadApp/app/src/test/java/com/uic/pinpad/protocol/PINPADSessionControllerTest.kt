...
import com.uic.pinpad.device.PinpadDeviceInfoProvider
...
class PINPADSessionControllerTest {
        ...
        val customController = PINPADSessionController(
            deviceInfoProvider = object : FakeDeviceInfoProvider() {
                override fun firmwareVersion(part: Char, option: Char?) =
                    com.uic.pinpad.device.FirmwareVersion(version = "FW2", subVersion = "01", hash = "ABCDEF")
            },
            codec = codec,
        )
    ...
    fun firmwareVersionResponseNormalizesLegacyA10PrefixWithModel() {
        val customController = PINPADSessionController(
            deviceInfoProvider = object : FakeDeviceInfoProvider() {
                override fun firmwareVersion(part: Char, option: Char?) =
                    com.uic.pinpad.device.FirmwareVersion(version = "A104.14.186-g86bb067")
            },
            codec = codec,
        )
        val responses = customController.onInbound(
    ...
    private open class FakeDeviceInfoProvider : PinpadDeviceInfoProvider {
        ...
        override fun firmwareVersion(part: Char, option: Char?): com.uic.pinpad.device.FirmwareVersion {
            return com.uic.pinpad.device.FirmwareVersion(version = if (part == '4') "APP0.2.0" else "FW1")
        }
